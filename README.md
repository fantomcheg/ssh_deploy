...
- Repository packages: Alacritty, Chromium, Code, iptux, OpenJDK 21, AMD firmware, Python pip, DNS/network utilities, Wireshark, Nmap/Zenmap, Gobuster, Hashcat, Hydra, John, Masscan, Metasploit, RustScan, SQLMap, tcpdump, WPScan, and ZAP.
...
- GenMachine HDMI setup: set `PVCLUB_FIX_GENMACHINE_HDMI=true` only on affected PCs to add the forced 1080p HDMI-A-1 and HDMI-A-2 kernel options to `/etc/kernel/cmdline` once and rebuild EndeavourOS systemd-boot entries with `reinstall-kernels`.
